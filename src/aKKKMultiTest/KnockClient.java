...
public class KnockClient extends JFrame implements ActionListener
{
    // KK protocol strings
    ...
    // GUI elements
    private JTextField hostField = new JTextField("sciris.shu.edu");
    private JTextField portField = new JTextField("1026");
    ...
    public void getJoke()
    {
        try
        {
            ...
            if (response.startsWith(KK_OK))
                numJokes = Integer.parseInt(response.substring(KK_OK.length()));
            else
    ...
    {
        KnockClient kc = new KnockClient();
        kc.setVisible(true);
    }
}
